function [vRecord,vthRecord,iappRecord] = loopUpdateAEVLIFNetCPU_recordVars(V,Vreset,tau_ref,Vth,Vth0,Vth_max,Isra,tau_sra,a,b,VsynE,VsynI,GsynE,GsynI,...
...
for i=1:nT
    ...
    areSimSpikes = any(spiked); % determine if there were any simulated spikes
    areAnySpikes = any(allSpikes); % determine if ther were any simulated or generated spikes
    if (areSimSpikes) % If there are any simulated spikes
        V(spiked) = Vreset(spiked); % reset membrane voltages of spiking neurons
        Vth(spiked) = Vth_max(spiked); % set the spike threshold to max
